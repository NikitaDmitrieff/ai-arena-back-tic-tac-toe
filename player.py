...
from prompts import ERROR_RECOVERY_PROMPT, SYSTEM_PROMPT, get_move_prompt
...
class Player:
    ...
    def __init__(
        ...
        if use_llm and agent is None:
            try:
                ...
                print(
                    f"Warning: Could not create LLM agent: {e}. Falling back to random moves."
                )
                self.use_llm = False
    ...
    def choose_move(
        self, available_moves: list, board_state: Optional[list] = None
    ...
    def _choose_move_llm(
        self, available_moves: list, board_state: list
    ) -> Tuple[Optional[Tuple[int, int]], Dict[str, Any]]:
        """
    ...
    def _parse_llm_response(
        self, response: str, available_moves: list
